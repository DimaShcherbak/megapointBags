...
namespace lib\internal\Mageplaza\Hybrid\Thirdparty\OpenID;

use Hybridauth\Thirdparty\OpenID\Identity;
use lib\internal\Mageplaza\Hybrid\Exception\Exception;
use lib\internal\Mageplaza\Hybrid\Exception\ExceptionInterface;
...
class LightOpenID
{
     ...
     * @param null $proxy
     *
     * @throws \Hybridauth\Thirdparty\OpenID\ErrorException
     */
    public function __construct($host, $proxy = null)
    {
        ...
        if (!function_exists('curl_init') && !in_array('https', stream_get_wrappers())) {
            throw new \Hybridauth\Thirdparty\OpenID\ErrorException('You must have either https wrappers or curl enabled.');
        }
    }
        ...
        if (!empty($proxy)) {
            ...
            if ($proxy && !empty($proxy['host'])) {
                ...
                if (array_key_exists('port', $proxy)) {
                    ...
                    if ($proxy['port'] <= 0) {
                        throw new \Hybridauth\Thirdparty\OpenID\ErrorException('The specified proxy port number is invalid.');
                    }
                }
    ...
    protected function request_curl($url, $method='GET', $params=array(), $update_claimed_id=false)
    {
        ...
        if (curl_errno($curl)) {
            throw new \Hybridauth\Thirdparty\OpenID\ErrorException(curl_error($curl), curl_errno($curl));
        }
    ...
    protected function request_streams($url, $method='GET', $params=array(), $update_claimed_id=false)
    {
        if (!$this->hostExists($url)) {
            throw new \Hybridauth\Thirdparty\OpenID\ErrorException("Could not connect to $url.", 404);
        }
    ...
    {
        if (!$url) {
            throw new \Hybridauth\Thirdparty\OpenID\ErrorException('No identity supplied.');
        }
        ...
        for ($i = 0; $i < 5; $i ++) {
            ...
            throw new \Hybridauth\Thirdparty\OpenID\ErrorException("No OpenID Server found at $url", 404);
        }
        throw new \Hybridauth\Thirdparty\OpenID\ErrorException('Endless redirection!', 500);
    }
