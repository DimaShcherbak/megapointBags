...
class MassUpdater
{
    ...
    const MARGIN = 'masspriceupdater_setup/general/margin';
    const DOLLAR_EXCHANGE_RATE = 'masspriceupdater_setup/general/dollar_exchange_rate';
    ...
    public function getDollarExchangeRate()
    {
        $dollar_exchange_rate = $this->scopeConfig->getValue(self::DOLLAR_EXCHANGE_RATE, self::STORESCOPE);
        return (float)$dollar_exchange_rate;
    }

    public function getMargin($regularPrice)
    {
        $margin = $this->scopeConfig->getValue(self::MARGIN, self::STORESCOPE);
        return (float)($margin / 100 * $regularPrice);
    }
...
//            $specialPrice = (float)$productData->getData('special_price');
            ...
            // Извлекаем цены из коллекций Keyt и Luba
            $keytProduct = $this->keytFactory->create()->load($sku, 'sku');
            $lubaProduct = $this->productFactory->create()->load($sku, 'sku');

            $keytPrice = $keytProduct->getId() ? (float)$keytProduct->getData('price') : null;
            $lubaPrice = $lubaProduct->getId() ? (float)$lubaProduct->getData('price') : null;

            // Сравниваем цены и выбираем максимальную
            $regularPrice = (float)max($keytPrice, $lubaPrice);

            $specialPrice = (float)$productData->getData('special_price');
            $cost = (float)$regularPrice * $this->getDollarExchangeRate();
            ...
            foreach ($priceType as $type) {
                ...
                if ($type == 'price' && $regularPrice) {
                    if ($updaterType == 'fixed') {
                        $newFixRegularPrice = ceil(($regularPrice + $this->getPriceAmount() + $this->getMargin($regularPrice)) * $this->getDollarExchangeRate());
                        $this->action->updateAttributes([$productData->getId()], ['price' => $newFixRegularPrice], $this->getStoreId());
                        $this->action->updateAttributes([$productData->getId()], ['cost' => $cost], $this->getStoreId());
                    } else {
